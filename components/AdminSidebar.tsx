...
import { Users, UserPlus, Calendar, LayoutDashboard, ChevronLeft, ChevronRight, AlertCircle, MessageSquare, CalendarDays, Sparkles } from 'lucide-react'

interface AdminSidebarProps {
  activeTab: 'dashboard' | 'leads' | 'unvalidated' | 'no-feedback' | 'calendar' | 'users'
  onTabChange: (tab: 'dashboard' | 'leads' | 'unvalidated' | 'no-feedback' | 'calendar' | 'users') => void
  onAddUser: () => void
}

export default function AdminSidebar({ activeTab, onTabChange, onAddUser }: AdminSidebarProps) {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      label: 'RDV sans retour',
      icon: MessageSquare,
      gradient: 'from-rose-500 via-red-500 to-accent-500',
      hoverGradient: 'from-rose-600 via-red-600 to-accent-600',
      bgColor: 'bg-rose-50',
    ...
    },
    {
      ...
      label: 'Utilisateurs',
      icon: Users,
      gradient: 'from-accent-500 via-brand-1000 to-brand-500',
      hoverGradient: 'from-accent-600 via-brand-700 to-brand-teal',
  ...
  return (
    <>
      ...
      >
        ...
        {/* Add User Button avec effet premium */}
        <div className="absolute bottom-6 left-0 right-0 px-4">
          <div className="relative group">
            ...
            >
              ...
              <UserPlus className={`h-6 w-6 ${collapsed ? '' : 'mr-2'} transform group-hover:rotate-90 transition-transform duration-300`} />
              {!collapsed && (
                <span className="relative z-10 tracking-wide">Ajouter Utilisateur</span>
              )}
            </button>
          </div>
        </div>
